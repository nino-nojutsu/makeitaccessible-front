...
const getPdfLines = (audit) => {
  const results = audit?.results || {};
  const summary = results.summary || {};
  const tests = Array.isArray(audit?.tests) ? audit.tests : [];
  const createdAt = results.createdAt ? new Date(results.createdAt) : null;
  const formattedDate = createdAt && !Number.isNaN(createdAt.getTime())
    ? createdAt.toLocaleString("fr-FR")
    : "Date inconnue";

  const lines = [
    "MakeItAccessible - Rapport d'audit RGAA",
    "",
    `Site audite : ${results.url || audit?.website?.domain || "Non renseigne"}`,
    `Date : ${formattedDate}`,
    `Score global : ${summary.score ?? 0}%`,
    `Anomalies : ${summary.violations ?? 0}`,
    `Tests a verifier : ${summary.incomplete ?? 0}`,
    `Criteres valides : ${summary.passes ?? 0}`,
    `Criteres non applicables : ${summary.inapplicable ?? 0}`,
    `Total criteres : ${summary.total ?? 0}`,
    "",
  ...
  tests.forEach((test) => {
    lines.push(
      `- ${test.category || "Sans categorie"} : ${test.violations?.length || 0} anomalie(s), ${test.incomplete?.length || 0} a verifier, ${test.passes?.length || 0} valide(s)`,
    );
  });

  lines.push("", "Anomalies detectees");
  ...
  tests.forEach((test) => {
    ...
    lines.push("", test.category || "Sans categorie");

    violations.forEach((violation) => {
      lines.push(
        `- ${violation.help || violation.description || violation.id || "Anomalie"} (${violation.impact || "impact inconnu"}) - ${getRuleCount([violation])} occurrence(s)`,
      );
    });
  });
...
const buildPdf = (lines) => {
  ...
  pages.forEach((pageLines) => {
    const stream = [
      "BT",
      `/F1 11 Tf`,
      `${marginX} ${firstLineY} Td`,
      ...pageLines.flatMap((line, index) => {
        const safeLine = normalizePdfText(line);
        return index === 0
          ? [`(${safeLine}) Tj`]
          : [`0 -${lineHeight} Td`, `(${safeLine}) Tj`];
      }),
      "ET",
    ].join("\n");
...
const buildCsv = (audit) => {
  const results = audit?.results || {};
  const summary = results.summary || {};
  const tests = Array.isArray(audit?.tests) ? audit.tests : [];
  const rows = [
    ["type", "categorie", "critere", "impact", "occurrences", "score", "url", "date"],
    [
      "resume",
      "",
      `Anomalies: ${summary.violations ?? 0} | A verifier: ${summary.incomplete ?? 0} | Valides: ${summary.passes ?? 0} | Non applicables: ${summary.inapplicable ?? 0} | Total: ${summary.total ?? 0}`,
      "",
      "",
      summary.score ?? 0,
      results.url || audit?.website?.domain || "",
      results.createdAt || "",
    ],
  ];
  ...
  tests.forEach((test) => {
    ...
    (test.incomplete || []).forEach((rule) => {
      rows.push([
        "a_verifier",
        test.category || "",
        rule.help || rule.description || rule.id || "",
        rule.impact || "",
        getRuleCount([rule]),
        "",
        results.url || "",
...
function HeroAudit({ isArchive }) {
  ...
  const handleDownloadPDF = () => {
    const pdf = buildPdf(getPdfLines(audit));
    const blob = new Blob([pdf], { type: "application/pdf" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    const auditId = audit?.results?._id || "audit";

    link.href = url;
    link.download = `rapport-${auditId}.pdf`;
  ...
  const handleDownloadCSV = () => {
    ...
    link.href = url;
    link.download = `rapport-${auditId}.csv`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
      ...
      <div className={styles.scoreCard} aria-label="Score global et export">
        ...
        {user.token ? (
          <section aria-label="Options d'export" className={styles.download}>
            <button className={styles.downloadPDF} type="button" onClick={handleDownloadPDF}>
              <svg
                width={20}
                height={20}
                aria-hidden="true"
            ...
            <p>— OU —</p>
            <button className={styles.downloadCSV} type="button" onClick={handleDownloadCSV}>
              <svg
                width={20}
                height={20}
                aria-hidden="true"
