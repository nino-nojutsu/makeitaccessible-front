...
function HeroAudit({ isArchive }) {
  ...
  return (
    <section
      className={styles.analyseHero}
      aria-label="Relancer un audit et voir le score global de mon audit"
    >
      {/* Si on est pas sur page audit cliquée depuis le dashboard, on affiche la barre de recherche et on appelle le composant analyse */}
      {!isArchive && (
      ...
      <div className={styles.scoreCard} aria-label="Score global et export">
        ...
        <section className={styles.centerSection}>
          <time className={styles.date}>
            Audit du 
            {new Date(audit?.results?.createdAt).toLocaleDateString("fr-FR", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })} 
          ...
          <p
            ...
            aria-label={`Site audité : ${audit.results.url}`}
          >
            {audit.results.url}
          </p>
          {user.token && (
            <button className={styles.auditButton} type="button">
              Voir audit complet
            </button>
          )}
          <div
          ...
          >
            ...
            <p className={styles.tooltipText} role="tooltip">
              l'audit ne porte jamais sur toutes les pages d'un site, mais sur
              un échantillon représentatif de 15 à 20 pages minimum (plus sur
              les sites complexes). Pour des sites plus volumineux/complexes, on
              monte à un échantillon de 40 à 80 pages.
            </p>
          </div>
        </section>
