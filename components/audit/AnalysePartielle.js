...
function AnalysePartielle() {
    ...
    return (
        <>
            <section aria-label="Accès à l'analyse complète" className={styles.mainAnalysePartielle}>
                <div>
                    <h2>Vous souhaitez voir la suite de votre analyse ?</h2>
                      <p>Connectez-vous pour accéder au détail complet : résultats par critère RGAA, par type (violations, éléments incomplets, validés) et par niveau de criticité !</p>
                </div>

                <section aria-label="Connexion ou inscription" className={styles.download}>
                    <button type="button" aria-controls="modal-signin" aria-haspopup="dialog" aria-expanded={signIn} className={headerStyles.btnConnected} onClick={() => setSignIn(true)}>
                        Se connecter
                    </button>
                    <p>Vous n’avez pas encore encore de compte ?</p>
                    <button type="button" aria-controls="modal-signup" aria-haspopup="dialog" aria-expanded={signUp} className={headerStyles.btnRegistration} onClick={() => setSignUp(true)}>
                        S'inscrire
                    </button>
                </section>
            </section>
