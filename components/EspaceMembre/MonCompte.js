...
function MonCompte() {
    ...
    const handleDelete = () => {
        Modal.confirm({
            ...
            cancelButtonProps: { style: { borderColor: '#111111', color: '#111111' }},
            onOk: () => {
            ...
            <div className={styles.layout}>
                ...
                <div className={styles.box}>
                    <div className={styles.field}>
                        <label>Nom</label>
                        <input type="text" placeholder={`Modifier votre nom : ${user.lastName}`} onChange={(e) => setModifyLastName(e.target.value)} value={modifyLastName} className={styles.inputSignUp} />
                    </div>
                    <div className={styles.field}>
                        <label>Prénom</label>
                        <input type="text"  placeholder={`Modifier votre prénom : ${user.firstName}`} onChange={(e) => setModifyFirstName(e.target.value)} value={modifyFirstName} className={styles.inputSignUp} />
                    </div>
                    <div>
                        <label className={styles.field}>Email</label>
                        <input type="email" placeholder={`Modifier votre email : ${user.email}`} onChange={(e) => setModifyEmail(e.target.value)} value={modifyEmail} className={styles.inputSignUp} />
                    </div>
                    <div>
                        <label className={styles.field}>Nom d'utilisateur</label>
                        <input type="text" placeholder={`Modifier votre nom d'utilisateur : ${user.username}`} onChange={(e) => setModifyUsername(e.target.value)} value={modifyUsername} className={styles.inputSignUp} />
                    </div>
                    <div>
                        <label className={styles.field}>Mot de passe</label>
                        <input type="password" placeholder="Modifier votre mot de passe" onChange={(e) => setModifyPassword(e.target.value)} value={modifyPassword} className={styles.inputSignUp} />
                    </div>
                    ...
                    <button className={styles.buttonDelete} onClick={() => handleDelete()}>Supprimer mon compte</button>
                </div>
            </div>
        </div>
    );
